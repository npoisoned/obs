...
def main():
    ...
    while True:
        ...
        memory_copy = copy(memory)
        if memory:
            for scene_name in memory_copy:
                if scene_name not in data:
                    client.remove_scene(scene_name)
                    memory.pop(scene_name)
                memory_scene_copy = memory[scene_name]
                for item in memory_scene_copy:
                    if item not in data[scene_name]:
                        client.remove_scene_item(scene_name, item['id'])
                        data[scene_name].pop(item)
